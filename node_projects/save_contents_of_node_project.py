...
def save_project_contents(root_dir, output_file):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        f.write("This document contains a detailed look at the files within this project, excluding build directories, node_modules, and any files listed in .gitignore.\n\n")
        ...
        for subdir, dirs, files in os.walk(root_dir):
            # Skip unwanted directories
            dirs[:] = [d for d in dirs if d not in {'node_modules', 'build'} and not gitignore_spec.match_file(os.path.relpath(os.path.join(subdir, d), root_dir))]
            ...
            for file in files:
                ...
                if file.endswith(('.js', '.html', '.css', '.md', '.txt', '.sh')):
                    f.write(f"### {file_path}\n")
